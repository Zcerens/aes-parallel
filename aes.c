...
static void mix_columns(uint8_t* s) {
    for (int i = 0; i < 16; i+=4) {
        ...
        uint8_t r = a ^ b ^ c ^ d;
        uint8_t x = a ^ b; x = xtime(x); s[i] ^= x ^ r;
        x = b ^ c; x = xtime(x); s[i+1] ^= x ^ r;
        x = c ^ d; x = xtime(x); s[i+2] ^= x ^ r;
        x = d ^ a; x = xtime(x); s[i+3] ^= x ^ r;
    }
}
...
void AES_key_expansion(uint8_t* key, uint8_t* roundKeys) {
    // basit key-schedule (test amaçlı)
